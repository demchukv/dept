...
const disconnectCertificateShema = z.object({
  id: z.number().min(1),
});
interface DisconnectCertificateProps {
  onClose: (state: boolean, e: React.MouseEvent | undefined) => void;
  data: certificateType;
}
export const CertificateCancel = ({
  onClose,
  data,
}: DisconnectCertificateProps) => {
  const form = useForm<z.infer<typeof disconnectCertificateShema>>({
    resolver: zodResolver(disconnectCertificateShema),
    mode: 'onChange',
    defaultValues: {
      id: data.id,
    },
  ...
  function onSubmit(data: z.infer<typeof disconnectCertificateShema>) {
    startTransition(() => {
      //TODO: make API request and setData
      // const newData = getJson('/data/call-summary.json');
      const values = { ...data, action: 'cancelCertificate' };
      toast({
      ...
      >
        ...
        <ModalInner className="flex items-center w-full font-normal text-base text-main-dark leading-main-lh space-y-4 self-center flex-grow">
          ...
          <p className="mb-3">
            ...
            <span className="font-medium">{data.cert}?</span>
          </p>
        </ModalInner>
