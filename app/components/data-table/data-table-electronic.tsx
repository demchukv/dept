...
export function DataTable<TData, TValue>({
  ...
  const category = table.getHeaderGroups()[0].headers.find((header) => {
    return header.id === 'category';
  });
  const department = table.getHeaderGroups()[0].headers.find((header) => {
    return header.id === 'department';
  });
  const user = table.getHeaderGroups()[0].headers.find((header) => {
    return header.id === 'user';
  });

  return (
    <>
      {/* <div className="mb-6 sm:mb-8 flex flex-col-reverse sm:flex-row sm:gap-6 sm:justify-between"> */}
      <div className="flex flex-col justify-between gap-2 sm:flex-row sm:gap-6 mb-4 sm:mb-9">
        {category?.column.getCanFilter() ? (
          <div className="w-full flex flex-col gap-2">
            <span className="text-xs leading-none">Категорія</span>
            <Filter
        ...
        {department?.column.getCanFilter() ? (
          <div className="w-full flex flex-col gap-2">
            <span className="text-xs leading-none">Відділ</span>
            <Filter
        ...
        {user?.column.getCanFilter() ? (
          <div className="w-full flex flex-col gap-2">
            <span className="text-xs leading-none">Користувач</span>
            <Filter
              column={user.column}
              placeholder="Оберіть зі списку"
              defSelectValue="Всі"
                  ...
                  <TableRow key={headerGroup.id} className="border-transparent">
                    {headerGroup.headers.map((header) => {
                      return (
                        <TableHead
                          key={header.id}
                          className="font-normal text-sm leading-main-lh text-gray-dark bg-white px-1 py-[22px] first:pl-2 last:pr-2 sm:first:pl-8 sm:last:pr-8 border-b-0"
                        >
                          {header.isPlaceholder
                            ? null
                            : flexRender(
                    ...
                    {table.getRowModel().rows?.length ? (
                      table.getRowModel().rows.map((row) => (
                        <TableRow
                          ...
                          className="odd:bg-bg-color border-transparent"
                        >
