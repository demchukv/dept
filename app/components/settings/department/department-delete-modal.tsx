...
const departmentDeleteSchema = z.object({
  id: z.number(),
...
export const DepartmentDeleteModal = ({
  ...
  const form = useForm<z.infer<typeof departmentDeleteSchema>>({
    resolver: zodResolver(departmentDeleteSchema),
    mode: 'all',
    defaultValues: {
      id: data.id,
  ...
  const onSubmit = (data: z.infer<typeof departmentDeleteSchema>) => {
    const values = { ...data };
    console.log(values);
  };
  ...
  return (
    <>
      <Button
        ...
        variant="ghost"
        className={cn(
          ' hover:shadow-none text-warning hover:text-main-dark gap-2 py-2.5 text-base',
          className,
        )}
                    ...
                    render={({ field }) => (
                      <FormItem className="mb-4 space-y-0">
                        ...
                        >
                          <FormControl>
                            <SelectTrigger>
                              <SelectValue placeholder="Обрати відділ для передачі" />
                            </SelectTrigger>
                          </FormControl>
                          <SelectContent>
                            {transEmail.map((item) => (
